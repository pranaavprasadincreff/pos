...
export function useOrderForm({ onSuccess, orderToEdit }: UseOrderFormProps) {
  ...
  const initializeRows = useCallback(async () => {
    if (orderToEdit?.items?.length) {
      const initializedRows: InputRow[] = await Promise.all(
        orderToEdit.items.map(async (item) => {
          try {
            ...
            // backend stores per-unit sellingPrice; UI wants total
      ...
      setRows(initializedRows.length ? initializedRows : [emptyRow()])
    } else {
      setRows([emptyRow()])
    }
    setFormError(null)
  ...
  const updateRow = useCallback(
    <K extends keyof InputRow>(index: number, field: K, value: InputRow[K]) => {
  ...
  const handleBarcodeChange = useCallback(
    (index: number, value: string) => {
      ...
      // UX: clear only invalid-barcode while typing
      setRows((prev) => {
        const r = prev[index]
        if (!r) return prev
      ...
      const timer = setTimeout(async () => {
        ...
        try {
          ...
          setRows((prev) => {
            ...
            // if user already entered a price, keep it; else default to mrp
  ...
  const handleSubmit = async (): Promise<boolean> => {
    ...
    const items: OrderCreateItemForm[] = rows.map((row) => {
      ...
      return {
        productBarcode: row.productBarcode.trim(),
        quantity: qty,
    ...
    try {
      ...
      if (orderToEdit?.orderReferenceId) {
        const payload: OrderUpdateForm = {
          orderReferenceId: orderToEdit.orderReferenceId,
          items,
        }
  ...
  useEffect(() => {
    if (orderToEdit) void initializeRows()
